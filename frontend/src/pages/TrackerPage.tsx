...
export const TrackerPage: React.FC = () => {
  ...
  const SortHeader: React.FC<{ label: string; sortField: SortKey }> = ({ label, sortField }) => (
    ...
    >
      <span>{label}</span>
      <ArrowUpDown className={`w-3 h-3 ${sortKey === sortField ? 'text-blue-600' : 'text-slate-300'}`} />
    </button>
  );
      ...
      <div className="pb-6 border-b border-slate-200 flex items-center justify-between flex-wrap gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition cursor-pointer"
          >
                ...
                {visibleApplications.map((app) => (
                  <tr key={app.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60 transition">
                    <td className="px-5 py-3.5">
                      <Link to={`/tracker/${app.id}`} className="font-semibold text-slate-900 hover:text-blue-700 transition">
