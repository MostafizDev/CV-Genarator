...
const TemplateCard: React.FC<TemplateCardProps> = ({
  ...
  <div
    className={`rounded-2xl border p-3 space-y-2.5 transition ${
      selected ? 'border-blue-400 ring-2 ring-blue-100 bg-blue-50/40' : 'border-slate-200 bg-white'
    }`}
  >
    ...
    <div className="flex items-center justify-between gap-2">
      <div className="min-w-0">
        ...
        <div className="flex items-center space-x-1.5 mt-0.5">
          {!template.is_custom ? (
            <span className="flex items-center space-x-1 text-[10px] px-1.5 py-0.5 bg-green-100 text-green-700 border border-green-200 rounded-full font-medium">
      ...
      {onDelete && (
        <button
          ...
          onClick={onDelete}
          disabled={deleting}
          className="shrink-0 p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition disabled:opacity-50"
      ...
      <div className="mt-10 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-mono leading-relaxed focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={creating}
              className="flex items-center space-x-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-semibold rounded-lg shadow-sm shadow-blue-600/20 transition disabled:opacity-60"
            >
          ...
          <div>
            ...
            {draftPreviewError ? (
              <div className="flex items-start space-x-2 p-3 rounded-lg bg-red-50 border border-red-200 text-red-800 text-xs">
